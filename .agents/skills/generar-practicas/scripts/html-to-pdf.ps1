<#
.SYNOPSIS
    Convierte una hoja de práctica o proyecto técnico HTML a PDF profesional listo para entrega/impresión A4.
...
.EXAMPLE
    .\html-to-pdf.ps1 -HtmlPath "01-Modulos/Acceso-a-Datos/03-Ejercicios/Evaluacion-Inicial/PRAC-AD-Evaluacion-Inicial.html"
#>
...
Write-Host "[*] Convirtiendo Práctica HTML a PDF (A4)..." -ForegroundColor Cyan
Write-Host "    Origen:  $fullHtmlPath"
Write-Host "    Destino: $PdfPath"
Write-Host "    Motor:   $browserExe"

# 5. Ejecutar conversión headless con opciones de calidad A4
...
if (Test-Path $PdfPath) {
    ...
    Write-Host "[+] Práctica PDF generada correctamente: $PdfPath ($sizeKb KB)" -ForegroundColor Green
    exit 0
} else {
    Write-Error "Error: El proceso finalizó pero no se generó el archivo PDF."
    exit 1
}
